...
# ICMP (Ping) logging and rate-limit
iptables -A INPUT -p icmp --icmp-type 8 -m conntrack --ctstate NEW -j LOG --log-prefix "SECURETIK-ICMP: " --log-level 7
iptables -A INPUT -p icmp --icmp-type 8 -m limit --limit 5/s --limit-burst 5 -j ACCEPT

# Accept ICMP Echo Reply from responses
...
# Send traffic to NFQUEUE for Suricata to inspect (placed at the end)
